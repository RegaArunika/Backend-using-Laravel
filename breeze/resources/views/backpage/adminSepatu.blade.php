<x-admin-layout>
        <div class="w-full">
            <div class="flex">
                ...
                <form action="{{ route('produk.index') }}" method="GET" class="mt-6">
                    <h2 class="text-md text-bold font-bold">Search Sepatu</h2>
                    <div class="flex items-center"> <!-- Added 'items-center' class to center items vertically -->
                        <select id="id_produk" name="id_produk" class="bg-gray-50 border p-3 border-gray-300 text-gray-900 text-sm rounded-l-lg focus:ring-blue-500 focus:border-blue-500 block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            @foreach($toko as $item)
                                <option value="{{ $item->id_produk }}" {{ (isset($_GET['id_produk']) && $_GET['id_produk'] == $item->id_produk) ? 'selected' : '' }}>{{ $item->nm_toko }}</option>
                            @endforeach
                        </select>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mt-10 border-collapse border border-gray-400 table-auto">
                ...
                <tbody>
                    ...
                    <tr class="bg-white  border border-gray-400">
                        <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border border-gray-400">
                            {{ $item->nama_produk}}
                        </td>
                        <td class="px-6 py-4 border border-gray-400">
                            <div>
                                <img src="{{ $item->img_produk }}" width="60px" height="60px" class="rounded-lg inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-soft-in-out h-9 w-9" alt="user1" />
                              </div>
                        </td>
                        ...
                        <td class="px-6 py-4 flex  ">
                            <a href="{{ route('produk.edit', $item->id_produk) }}"  class="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-2">
                              ...
                              <form action="{{ route('produk.destroy', $item->id_produk) }}" class="font-medium text-red-600 dark:text-blue-500 hover:underline mr-2
                                 ml-3" method="POST">
                                  @csrf @method('DELETE')
